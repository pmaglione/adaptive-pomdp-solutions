...
int main (int argc, char *argv[])
{
	...
	//These used to be 1
	//double prioralpha[2] = {1.0, 0.5};
	int NUMBER_OF_WORKER_POOLS;

	FILE *fi = fopen("../simulation.info", "r");
    fscanf(fi, "%d\n", &NUMBER_OF_WORKER_POOLS);

	double *prioralpha = (double *) malloc(sizeof(double) * NUMBER_OF_WORKER_POOLS);

    double temp1,temp2,temp3;
    fscanf(fi,"%lf\n",&temp1);
    fscanf(fi,"%lf,%lf\n",&temp2,&temp3);
    fscanf(fi,"%lf,%lf\n",&prioralpha[0],&prioralpha[1]);
    fclose(fi);
   ...
   //This code will now break, if extra arguments are passed, since prioralpha is
    // now an array.
  /*
  if ( argc > 2 )
